...
- Built-in sub-agent context injects `AGENTS.md`, `SOUL.md`, and `IDENTITY.md`, preserving operating instructions and identity without sharing `USER.md`, `MEMORY.md`, or `BOOTSTRAP.md`. Its `## Tools` section carries environment-specific notes. Codex-native subagents currently use Codex's native `AGENTS.md` discovery boundary; parent persona, identity, and user files remain turn-scoped collaboration instructions and are not inherited by native children.
